    ...
    public static void init(IConfig config) {
        ...
        if (comboPooledDataSource == null) {
            ...
            if (IsEmbedDB) {
                ...
                String migrate = config.getProperty(BrokerConstants.FLY_WAY_PROPERTY_NAME);
                migrateLocation = StringUtil.isNullOrEmpty(migrate) ? "filesystem:./migrate/mysql" : migrate;
                ...
                try {
                    ...
                    // 连接已经存在的数据库，如：mysql
                    Connection connection = DriverManager.getConnection(url02, comboPooledDataSource.getUser(),
                        comboPooledDataSource.getPassword());
                    Statement statement = connection.createStatement();

                    // 创建数据库
                    statement.executeUpdate(
                        "CREATE DATABASE IF NOT EXISTS `" + datasourceName + "` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;");
